...
import ChainedMap from '../lib/chained-map'
...
test('ordering before', () => {
  ...
  expect(map.values().map(o => o.values())).toStrictEqual([['alpha'], ['beta']])
})
...
test('ordering before and after', () => {
  ...
  expect(map.values().map(o => o.values())).toStrictEqual([['alpha'], ['beta'], ['gamma']])
})
