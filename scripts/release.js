...
async function main() {
  if (!targetVersion) {
    ...
    const currentVersion = (pkgInfoList.find(pkg => pkg.isPublic) || {}).pkgInfo.version
    // inquirer questions
    const questions = [
      {
        type: 'list',
  ...
  if (stdout) {
    ...
    await runIfNotDry('git', ['commit', '-m', `chore(release): publish v${targetVersion}`])
  } else {
...
function updateVersion() {
  ...
  const dependencyTypeList = ['dependencies', 'devDependencies', 'optionalDependencies', 'peerDependencies']
...
async function publishPackages() {
  step('\nPublishing package...')
  const publicArgs = ['publish', '[folderSlot]', '--access', 'public', '--registry', NPM_DEFAULT_REGISTRY]
  if (tagBetaId) {
